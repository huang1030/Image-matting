...
def make_test_data_list(x_data_path): #make_test_data_list函数得到测试中的x域的图像路径名称列表
    x_input_images = glob.glob(os.path.join(x_data_path, "*")) #读取全部的x域图像路径名称列表
    return x_input_images
 
def get_picture(x_image, fake_y): #get_write_picture函数得到网络测试结果
    x_image = cv_inv_proc(x_image) #还原x域的图像
    fake_y = cv_inv_proc(fake_y[0]) #还原生成的y域的图像
    output = np.concatenate((x_image, fake_y), axis=1)  
    return output

def get_write_picture(x_image, y_image, fake_y):#get_write_picture函数得到训练过程中的可视化结果
    x_image = cv_inv_proc(x_image) #还原x域的图像
